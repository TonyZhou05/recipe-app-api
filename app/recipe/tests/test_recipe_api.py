...
class PrivateRecipeAPITests(TestCase):
    ...
    def test_create_recipe(self):
        ...
        # create a post request using the apiclient to create a recipe
        res = self.client.post(RECIPES_URL, payload)

        self.assertEqual(res.status_code, status.HTTP_201_CREATED)
        # get the recipe object with the id of the returned object id
        recipe = Recipe.objects.get(id=res.data['id'])
        for k,v in payload.items():
            # getattribute of k from recipe, check that is equal to the one we passed in
    ...
    def test_recipe_other_users_recipe_error(self):
        '''Test trying to delete anotherusers recipe gives error.'''
        new_user = create_user(email='user2@example.com', password='test123')
    ...
    def test_clear_recipe_tags(self):
        ...
        # assign an empty tag to the recipe object
        payload = {'tags': []}
        url = detail_url(recipe.id)
        res = self.client.patch(url, payload, format='json')

        # check that recipe does not have any tags anymore
        self.assertEqual(res.status_code, status.HTTP_200_OK)
